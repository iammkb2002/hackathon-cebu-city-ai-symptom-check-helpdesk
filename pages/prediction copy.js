...
const MedicalDiagnosisApp = () => {
    ...
    const predictDisease = async () => {
        try {
            ...
            console.log("Initial prediction response data:", data);

            // Send data to Gemini API for rechecking
            const geminiApiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
            const geminiPrompt = `
            Based on the following data, provide more accurate and comprehensive information. Ensure the format includes Disease, Probability, Description, and Precautions.
            ...
            const geminiResponse = await fetch('/api/gemini', {
                ...
                body: JSON.stringify({ prompt: geminiPrompt, geminiApiKey }),
            });

            if (!geminiResponse.ok) {
                throw new Error('Gemini API response was not ok');
            }

            const geminiData = await geminiResponse.json();
            console.log("Gemini response data:", geminiData);

            // Assume geminiData.response contains the correct JSON format
            const formattedPredictions = JSON.parse(geminiData.response);
            ...
            setPredictions([formattedPredictions].slice(0, 3)); // Display at most 3 cards
        } catch (error) {
        ...
        <div>
            <div className="min-h-screen flex flex-col items-center justify-center gradient">
                ...
                <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-2xl">
                    ...
                    >
                        {loading ? 'Loading...' : 'Predict Disease'}
                    </button>
